...
(() => {
  ...
  document.addEventListener("keydown", event => {
    const seletorHorario = document.getElementById("timePickerModal");
    if(event.key === "Escape" && seletorHorario && !seletorHorario.hidden) return;
  ...
  agendaForm.addEventListener("submit", event => {
    ...
    if(!eventoHoraInicio.value){
      window.alert("Defina o horário de início do compromisso.");
      eventoHoraInicio.click();
      return;
    }
